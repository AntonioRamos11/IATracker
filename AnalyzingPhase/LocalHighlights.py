...
# Add this class definition before the HighlightExtractionError
class DeepSeekSummarizer:
    ...
    def generate_highlights(self, paper_data, max_new_tokens=1024):
        """Generate highlights using local model"""
        inputs = None
        outputs = None
        try:
            prompt = self.create_dynamic_prompt(paper_data)
    ...
    def detect_sections(content):
        sections = {}
        common_headers = ['introduction', 'methods', 'results', 'discussion']
        for header in common_headers:
            sections[header] = header in content.lower()
        return sections
    
    def create_dynamic_prompt(self, paper_data):
        """Create adaptive prompt based on paper content analysis"""
        # Use the class method to detect sections
        sections = self.detect_sections(paper_data['content'])
        has_math = bool(re.search(r'\$(.*?)\$|\\begin{equation}', paper_data['content']))
        num_figures = len(re.findall(r'Fig\.|Figure', paper_data['content'], re.IGNORECASE))

        # Base template
        prompt = f"""Generate structured highlights for this {paper_data.get('doc_type', 'research paper')}:
        ...
        **Abstract**: {paper_data.get('abstract', '')}
        ...
        if num_figures > 3:
            prompt += "\n- Key visual findings from figures"
        # Example: check if 'experiment' key exists in sections
        if sections.get('introduction'):  
            prompt += "\n- Experimental setup and validation"
        # Avoid calling .lower() on a dictionary. Use appropriate logic here.
        if 'simulation' in ' '.join(sections.keys()).lower():
            prompt += "\n- Simulation parameters and numerical methods"

        # Static requirements...
        prompt += """
        **Required Format:**
        1. Core Innovation (1-2 sentences)
        2. Technical Approach (3-5 bullet points)
        3. Quantitative Findings (numbered list with metrics)
        4. Limitations & Assumptions
        5. Domain Impact (theoretical/practical)

        Include key equations/figures where critical"""
    ...
    def _parse_response(self, response_text):
        ...
        sections = {
            "contribution": self._extract_section("Main Contribution", response_text),
            "methods": self._extract_list("Key Methods", response_text),
            "results": self._extract_list("Significant Results", response_text),
            "limitations": self._extract_list("Limitations", response_text),
            "impact": self._extract_section("Practical Impact", response_text)
        }
        # Fallback: if all sections are empty, log and return raw response under 'full_response'
        if not any(sections.values()):
            self.logger.warning("Parsed output is empty. Returning raw response instead.")
            return {"full_response": response_text}
        
        return sections

    def _extract_section(self, header, text):
        """Extract section content following a header"""
        match = re.search(f"{header}:(.+?)(?=\\n\\d+\.|$)", text, re.DOTALL)
        return match.group(1).strip() if match else ""

    def _extract_list(self, header, text):
        """Extract bullet points from a section"""
        section = self._extract_section(header, text)
        return [line.strip("-• ").strip() for line in section.split("\n") if line.strip()]

# Initialize the summarizer at module level
...
def extract_highlights(json_path, api_url, api_max_tokens=None):
    try:
        ...
        section_markers = {
            ...
            "results": r"\b(Results|Findings|Analysis)\b",
        ...
        highlights["main_staff"] = {
            ...
            "sections_text": identify_sections(" ".join(highlights["topics"])),
        ...
        logger.info(f"✓ Successfully processed: {api_data.get('title', 'Unknown')} -> {highlight_path}")
        return highlights
