...
class ResearchQuestionAnswerer:
    """Class dedicated to answering research questions based on papers database - handles both highlights and raw text"""
    ...
    def load_papers(self):
        """Load papers from both processed_pdfs """
        logger.info("Loading papers from database...")
        
        # Load raw paper files
        paper_files = list(self.papers_dir.glob("*.json"))
        highlight_files = list(self.highlights_dir.glob("*_highlights.json"))
        
        # Track processed paper IDs to avoid duplicates
        processed_ids = set()
        
        # Process highlight files first (they contain structured data)
        for hf in highlight_files:
            ...
            try:
                ...
                # Check if this is a structured highlight format
                if "Core Innovation" in full_response:
                    ...
                    self.papers[paper_id] = {
                        "paper_type": "structured_highlights",
                    ...
                    # Handle non-structured highlights
                    self.papers[paper_id] = {
                        "paper_type": "non_structured_highlights",
                        "title": metadata.get("title", main_staff.get("title", paper_id)),
                        "authors": metadata.get("author", main_staff.get("authors", ["Unknown"])),
                        "published": metadata.get("published", "Unknown"),
                        "abstract": main_staff.get("abstract", ""),
        ...
        # Process raw paper files
        for pf in paper_files:
            paper_id = pf.stem
            if paper_id in processed_ids:
                continue  # Skip if already processed from highlights
            ...
            try:
                ...
                main_staff = paper_data.get("main_staff", {})
                
                # Initialize paper entry with raw text
                self.papers[paper_id] = {
                    "paper_type": "raw_text",
                    "title": metadata.get("title", main_staff.get("title", paper_id)),
                    "authors": metadata.get("author", main_staff.get("authors", ["Unknown"])),
        ...
        for idx in top_indices:
            ...
            if similarity_scores[idx] > 0.1:  # Threshold for relevance
                relevant_papers.append({
                    ...
                    "core_innovation": paper.get("core_innovation", ""),
                    "technical_approach": paper.get("technical_approach", ""),
                    "quantitative_findings": paper.get("quantitative_findings", ""),
                    "limitations": paper.get("limitations", ""),
                    "domain_impact": paper.get("domain_impact", ""),
